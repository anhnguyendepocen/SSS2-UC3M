...
visualizeFitLm3D <- function(data, nGrid = 5, theta = -30, phi = 20,
  ...
  panelFirst <- function(pmat) {
    ...
    # Plot linear models in the laterals
    if (lmLaterals) {
  ...
  gridMat <- scatter3D(data$X, data$Y, data$Z, pch = 16, theta = theta, 
  ...
  text(x = trans3d(gX[1], gY[nGrid], median(gZ), gridMat), labels = "y", 
       pos = 2)
  ...
  if (alpha > 0) {
    ...
    surf3D(x = M$x, y = M$y, z = matrix(zUp, nrow = lx, ncol = ly),
           col = "yellow", alpha = 0.1, add = TRUE, border = "yellow2")
    surf3D(x = M$x, y = M$y, z = matrix(zDown, nrow = lx, ncol = ly), 
           col = "yellow", alpha = 0.1, add = TRUE, border = "yellow2")
    
  }
  surf3D(x = M$x, y = M$y, z = matrix(pred, nrow = lx, ncol = ly), 
         col = "lightblue", alpha = 0.2, border = gray(0.5), add = TRUE)
