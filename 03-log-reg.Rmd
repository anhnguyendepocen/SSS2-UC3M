...
Since it is a bit cumbersome to compute by yourself the CIs, we can code the function `predictCIsLogistic` instead of, see below.
```{r, collapse = TRUE}
...
```{r, saturated, echo = FALSE, results = 'hide', out.width = '70%', fig.show = 'hold', fig.asp = 1, fig.align = 'center', fig.cap = 'Fitted logistc regression versus *a* saturated model (several are possible depending on the interpolation between points).'}
set.seed(45671231)
x <- runif(50, 0, 1)
...
legend("topright", legend = c("Fitted logistic model", "A saturated model"), lwd = 2, col = c(2, 4))
```
...
\]
Since the likelihood of the saturated model is exactly one^[The probability of the sample according to the saturated is one -- replace $p(\mathbf{X}_i)=Y_i$ in \@ref(eq:eq-lik).], then the deviance is simply another expression of the likelihood:
\[
D=-2\log\text{lik}(\hat{\boldsymbol{\beta}}).
\]
As as consequence, **the deviance is always larger or equal than zero, being zero only if the fit is perfect**. 

A benchmark for evaluating the magnitude of the deviance is the **null deviance**, 
\[
D_0=-2\log\text{lik}(\hat{\beta}_0),
\]
which is the deviance of the fitted model without any predictor, but just the intercept:
\[
Y|(X_1=x_1,\ldots,X_k=x_k)\sim \mathrm{Ber}(\mathrm{logistic}(\beta_0)).
\]
In this case, $\hat\beta_0=\mathrm{logit}(\frac{m}{n})=\log\frac{\frac{m}{n}}{1-\frac{m}{n}}$ where $m$ is the number of $1$'s in $Y_1,\ldots,Y_n$. 
...
- Is **not the percentage of variance explained by the logistic model**, but rather a ratio indicating how close is the fit to being perfect.
- It is not related to any correlation coefficient.

```

```{block2, type='rmdinsight'}
The $R^2$ in \@ref(eq:log-r2) is valid for the whole family of *generalized linear models*, for which linear regression is a particular case. The connexion between \@ref(eq:log-r2) and the determination coefficient is given by the expressions of the deviance and null the deviance for the linear model:
\[
...
summaryLog # 'Residual deviance' is the deviance; 'Null deviance' the null deviance
...
| Reality \ Classified | $\hat Y=0$ | $\hat Y=1$ |
|----------------------|------------|------------|
...
(one level and more than two levels)
...
knitr::include_app('https://egarpor.shinyapps.io/log-reg-non-lin/', height = '1000px')
```
-->
