...
- $\text{SSR}=\sum_{i=1}^n\left(\hat Y_i-\bar Y\right)^2$, the **regression sum of squares**^[Recall that SSR is different from RSS (Residual Sum of Squares).]. This is the variation explained by the regression line, that is, *the variation from $\bar Y$ that is explained by the conditional mean $\hat\beta_0+\hat\beta_1X_i$*. $\text{SSR}=ns_{\hat y}^2$, where $s_{\hat y}^2$ is the sample variance of $\hat Y_1,\ldots,\hat Y_n$.
- $\text{SSE}=\sum_{i=1}^n\left(Y_i-\hat Y_i\right)^2$, the **sum of squared errors**. Is the variation around the conditional mean. Recall that $\text{SSE}=\sum_{i=1}^n \hat\varepsilon_i^2=(n-2)\hat\sigma^2$, where $\hat\sigma^2$ is the sample variance of $\hat \varepsilon_1,\ldots,\hat \varepsilon_n$. 
...
```{r, transf, echo = FALSE, fig.cap = 'Illustration of the choice of the non-linear transformation.', screenshot.opts = list(delay = 10), dev = 'png', cache = TRUE, fig.align = 'center', out.width = '90%'}
...
- Load `datasets.RData`.
- Establish the case names in `la-liga-2015-2016` as the variable `Team`.
...
- Exit `R Commander` and load `newDatasets.RData`.
```
-->
...
- What is the covariance between `dRGDP` and `debtgdp`? What is the standard deviation of each variable?
...
- Compute the CIs for the coefficients. Can we conclude that the effect of `debtgdp` on `dRGDP` is positive? And negative? 
...
- Which assumptions of the linear model you think are satisfied? Should we trust blindly the results obtained assuming the assumptions were satisfied?
```
