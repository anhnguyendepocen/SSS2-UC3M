...
- $(\hat\beta_0,\hat\beta_1)$ are computed from the data, in particular, they are the output of `lm`. In an abuse of notation, the term *regression line* is often used to denote both the *theoretical* ($y=\beta_0+\beta_1x$) and the *estimated* ($y=\hat\beta_0+\hat\beta_1x$) regression lines.
```
...
```{block, type = 'rmdnote'}
Do the same checking for $X=$`ReadingMean` and $Y=$ `MathMean` from the `pisa` dataset. Then with $X=$`logGDPp`. 
```
