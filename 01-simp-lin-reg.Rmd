...
- In `wdi-countries`, create a new variable that standarizes^[The standarization of $X_1,\ldots,X_n$ is $Z_1,\ldots,Z_n$, with $Z_i=\frac{X_i-\bar X}{s_x}$, where $s_x$ is the standard deviation of $X_1,\ldots,X_n$] the `GDP.growth`. Call it `GDP.growth`.
...
- Identify the 5 most outlierish points `'Automatically'`.
...
```{block, type = 'rmdinsight'}
The last scatterplot is an illustration of the **Simpson's paradox**. The paradox surges when there are two or more well-defined groups in the data, they all have *positive (negative) correlation*, but taken as a *whole dataset*, the **correlation is the opposite**.
```
...
TBA
<!--
Identify the problems with the following `R` codes, compared with what they were meant to do. Work out the right answer.

- "Compute the mean of $-1.23, \sqrt{2}, 2.34, 5, 0.15$"
-->
```
...
- What is the justification for a vertical distance?
- Is $\sigma^2$ affecting 
-
-
-->
```
