...
```{r, ttest, echo = FALSE, out.width = '90%', fig.align = 'center', fig.cap = 'The Student\'s $t$ distribution for the $t$-statistics associated to $\beta_0=0$ and $\beta_1=0$, for the `y1 ~ x1` regression.'}
...
confint(mod1)
```
By default, the conficence interval is computed for $\alpha=0.05$. You can change this on the `level` argument, for example:
```{r}
...
A CI for $\sigma^2$ can be also computed, but is less important in practise. The formula is:
$$
