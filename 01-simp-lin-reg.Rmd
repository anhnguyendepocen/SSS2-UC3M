...
- **Variance**. The variances $\mathrm{SE}(\hat\beta_0)^2$ and $\mathrm{SE}(\hat\beta_1)^2$ have an interesting interpretation:
    ...
    - *Error variance $\sigma^2$*. The more disperse $Y$ is, the less precise the estimates are.
...
\begin{equation}
\left(\hat\beta_j\pm\hat{\mathrm{SE}}(\hat\beta_j)t_{n-2;\alpha/2}\right),\quad j=0,1,\label{eq:ci}
\end{equation}
...
## Prediction of new observations {#prediction}

The forecast of $Y$ from $X=x$ in the linear model is approached by two different approaches:
...
\begin{equation}
\left(\hat y \pm t_{n-2:\alpha/2}\sqrt{\frac{\hat\sigma^2}{n}\left(1+\frac{(x-\bar x)^2}{s_x^2}\right)}\right).\label{eq:ci1}
\end{equation}
...
\begin{equation}
\left(\hat y \pm t_{n-2:\alpha/2}\sqrt{\hat\sigma^2+\frac{\hat\sigma^2}{n}\left(1+\frac{(x-\bar x)^2}{s_x^2}\right)}\right)\label{eq:ci2}
\end{equation}
...
The prediction and computation of CIs can be done with the R function predict. The objects required for predict are: first, the output of lm; second, a data.frame containing the locations  x
  where we want to predict  β0+β1x
 . To illustrate the use of predict, we are going to use the pisa dataset. In case you do not have it loaded, you can download it here as a .RData file.
...
Before entering explaining ANOVA, is important to recall an interesting result: *the mean of the fitted values $\hat Y_1,\ldots,\hat Y_n$ is the mean of $Y_1,\ldots, Y_n$*. This is easily seen if we plug-in the expressions of $\hat\beta_0$ and $\hat\beta_1$:
$$
...
- $\text{SSR}=\sum_{i=1}^n\left(\hat Y_i-\bar Y\right)^2$, the **regression sum of squares**. This is the variation explained by the regression line, that is, *the variation from $\bar Y$ that is explained by the conditional mean $\hat\beta_0+\hat\beta_1X_i$*. $\text{SSR}=ns_{\hat y}^2$, where $s_{\hat y}^2$ is the sample variance of $\hat Y_1,\ldots,\hat Y_n$.
- $\text{SSE}=\sum_{i=1}^n\left(Y_i-\hat Y_i\right)^2$, the **sum of squared errors**. Is the variation around the conditional mean. Recall that $\text{SSE}=\sum_{i=1}^n \hat\varepsilon_i^2=ns_{\hat \varepsilon}^2$, where $s_{\hat \varepsilon}^2$ is the sample variance of $\hat \varepsilon_1,\ldots,\hat \varepsilon_n$. 

The ANOVA decomposition is
$$
\text{SST} = \text{SSR} + \text{SSE}
$$
and its graphical interpretation is in Figure \@ref(fig:anova).
...
The ANOVA table summarizes the decomoposition of the variance. Here is given  in the form used by `R`.
...
$$
that is, the hypothesis of no linear dependence of $Y$ on $X$. The result of this test is completely equivalent to the $t$-test that we saw in Section \@ref(inference) (but it will have a different meaning in the next chapter). Under $H_0$,
$$
F=\frac{\text{SSR}/1}{\text{SSE}/(n-2)}\stackrel{H_0}{\sim} F_{1,n-2},
$$
where $F_{1,n-2}$ is the *Snedecor's $F$ distribution*^[The $F_{n,m}$ distribution arises as the quotient of two independent random variables $\chi^2_n$ and $\chi^2_m$, $\frac{\chi^2_n/n}{\chi^2_m/m}$.] with $1$ and $n-2$ degrees of freedom. The $p$-value of this test is the same as the $p$-value of the $t$-test for $H_0:\beta_1=0$.

Recall that the $F$-statistic, its $p$-value and dgerees of freedom are given in the output of `summary`.
...
$$
$R^2$ measures the **proportion of variation** of the response variable $Y$ that is **explained** by the predictor $X$ through the regression. The proportion of total variation of $Y$ that is not explained is $1-R^2=\frac{\text{SSE}}{\text{SST}}$. Intuitively, $R^2$ measures the **tightness of the data cloud around the regression line**, since is related directly with $\hat\sigma^2$.

The $R^2$ is related with the sample *correlation coefficient*
$$
r_{xy}=\frac{s_{xy}}{s_xs_y}=\frac{\sum_{i=1}^n \left(X_i-\bar X \right)\left(Y_i-\bar Y \right)}{\sqrt{\sum_{i=1}^n \left(X_i-\bar X \right)^2\sum_{i=1}^n \left(Y_i-\bar Y \right)^2}}
$$
and it can be seen that $R^2=r_{xy}^2$. Interestingly, it also holds that $R^2=r^2_{y\hat y}=\frac{s_{y\hat y}}{s_ys_{\hat y}}$, that is, *the square of the sample correlation coefficient between $Y_1,\ldots,Y_n$ and $\hat Y_1,\ldots,\hat Y_n$ is $R^2$*, a fact that is not inmediately evident. This fact can be visualized with the next code.
...
```
For example, the following code shows two models equally valid, but with different $R^2$. The first gives more accurate predictions since the error variance is lower.
```{r}
# Fixed design
x <- seq(0, 1, l = 50)
...
# R^2 depends on the error variance!
summary(reg1)
summary(reg2)

# Plot
scatterplot(y1 ~ x, smooth = FALSE)
scatterplot(y2 ~ x, smooth = FALSE)
```
...
Here is a counterexample of a linear regression done in a dataset that clearly does not satisfy the assumptions discussed in Section \@ref(assumptions), but despite so it has a large $R^2$.
```{r}
# Create data that: 
...
- [`data1.txt`](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/data1.txt)
- [`data2.csv`](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/data2.csv)
- [`data3.txt`](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/data3.txt)
- [`data4.xlsx`](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/data4.xlsx)
- [`data5.RData`](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/data5.RData)
- [`data6.txt`](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/data6.txt)
- [`data7.csv`](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/data7.csv)
- [`data8.RData`](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/data8.RData)
- [`data9.xlsx`](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/data9.xlsx)
- [`data10.txt`](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/data10.txt)

Create the file `datasets.RData` containing all the datasets.
```    
-->
...
<!--
Perform the following data management operations:

- Load `datasets.RData`.
- Establish the case names in `data1.txt` as the variable ``.
- Establish the case names in `data7.csv` as the variable ``.
...
- Set the observation
- Set the observation     
- Create a single `.RData` file saving all the modified datasets.
- Load `datasets.RData`.
-->
...
<!--
Compute the linear regression for:

- Compute the regression line for
- Scatterplot
- 
- 
--->
...
<!---
- Explain in your own words what is the operator `<-` doing.
...
- Set the positions 3, 4 and 8 of $x$ to 0. Set the positions 1, 4, 9 of $y$ to 0.5, -0.75, 0.3, -0.2.
- Create a new vector $z$ containing $\log(x^2) - y^3*\sqrt(\exp(x))$
- Create the vector $1, 4, 9, 16, 25, \ldots, 100$
- Matrix1
- Matrix2
- data.frame
- functions
...
- summary
- Obtain the $R^2$
- Check that $R^2$ is the same as $r_{xy}$ and $r_{}$
- Which are the most overrepresented countries? Which are the least?
- States
- 
-->
...
- Explain in your own words what is the difference between $(\beta_0,\beta_1)$ and $(\hat\beta_0,\hat\beta_1)$.
- Is $\hat\beta_0$ a random variable? What about $\hat\beta_1$? Justify your answer.
- What function are the least squares estimates minimizing? 
```
-->
...
<!--
```{block, type = 'rmdexercise'}
The dataset `moreAssumptions.RData` ([download](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/moreAssumptions.RData)) contains the variables `x1`, ..., `x10` and `y1`, ..., `y10`. For each regression `y1 ~ x1`, ..., `y10 ~ x10` describe whether the assumptions of the linear model are being satisfied or not. Justify your answer and state which assumption(s) are violated. 
```
-->
...
<!---
Quadratic, exponential, square-root or logarithmic trend?
-->
...
<!--
The paper *Growth in a time of debt* (@Reinhart2010a, @Reinhart2010b). You can read the story at [BBC](http://www.bbc.com/news/magazine-22223190),
[The New Yor Times](http://www.nytimes.com/2013/04/19/opinion/krugman-the-excel-depression.html) and 
[The Economist](http://www.economist.com/news/finance-and-economics/21576362-seminal-analysis-relationship-between-debt-and-growth-comes-under). Also, this [CNN interview to Paul Krugman](https://www.youtube.com/watch?v=3xj338ACri8) summarizes the key point (minutes 1:16 to 1:40).
...
The data is available [here](http://www.peri.umass.edu/236/hash/31e2ff374b6377b2ddec04deaa6388b1/publication/566/)
@Herndon2013

```{block, type = 'rmdexercise'}
Do:
- Import the data
- Compute and plot a linear regression
- 
```
-->
