...
# Simple linear regression {#simplin}
...
\]
which in Chapter \@ref(multlin) will be extended to multiple linear regression.

To convince you why simple linear regression is useful, let's begin by seeing what it can do in real-case scenarios!

## Examples and applications {#simplin-examps}

### Case study I: PISA scores and GDPp {#simplin-examps-pisa}
...
(ref:pisatabletitle) First 10 rows of the `pisa` dataset for a selection of variables. Note the `NA` (*Not Available*) in Chinese Taipei (or Taiwan).

```{r, pisatable, echo = FALSE, out.width = '90%', fig.align = 'center', fig.pos = 'h!', cache = TRUE}
pisa <- read.csv(file = "datasets/pisa.csv", header = TRUE)
knitr::kable(
  head(pisa[, c(1, 2, 5, 6, 8, 9)], 10),
  booktabs = TRUE,
  longtable = TRUE,
  caption = '(ref:pisatabletitle)'
)
```
...
(ref:viewtitle) Correct importation of the `pisa` dataset.

(ref:scatter1title) Scatterplot window, `'Data'` panel.

(ref:scatter2title) Scatterplot window, `'Options'` panel. Remember to tick the `'Least-squares line'` box in order to display the fitted regression line.
...
1. **Import the data into `R Commander`**.
    ...
    * Go to `'Data' -> 'Import data' -> 'from text file, clipboard, or URL...'`. A window like Figure \@ref(fig:read) will pop-up. Select the appropiate formatting options of the data file: whether the first row contains the name of the variables, what is the indicator for missing data, what is the field separator, and what is the decimal point character. Then click `'OK'`.
        ...
        ```{r, read, echo = FALSE, out.width = '35%', fig.align = 'center', fig.pos = 'h!', fig.cap = 'Data importation options.', cache = TRUE}
    ...
    * Since each row corresponds to a different country, we are going to name the rows as the value of the variable `Country`. To that end, go to `'Data' -> 'Active data set' -> 'Set case names...'` and select the variable `Country` and click `'OK'`. The dataset should look like Figure \@ref(fig:view).
        ...
        ```{r, view, echo = FALSE, out.width = '70%', fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:viewtitle)', cache = TRUE}
...
2. **Fit a simple linear regression**.
    ...
    * Go to `'Statistics' -> 'Fit models' -> 'Linear regression...'`. A window like Figure \@ref(fig:lm) will pop-up.
        ...
        ```{r, lm, echo = FALSE, out.width = '45%', fig.align = 'center', fig.pos = 'h!', fig.cap = 'Window for performing simple linear regression.', cache = TRUE}
        ...
        In our case $Y=$ `MathMean` and  $X=$ `logGDPp`, so select them and click `'OK'`^[In principle, you could pick more than one explanatory variables using the `'Control'` or `'Shift'` keys, but that corresponds to the *multiple linear regression* (covered in Chapter \@ref(multlin)).].
    ...
    * The window in Figure \@ref(fig:lm) generates this code and output:
        ...
        This is the linear model of `MathMean` regressed on `logGDPp` (first line) and its summary (second line). The summary gives the coefficients of the line and the $R^2$ (`'Multiple R-squared'`), which -- as we will see in Section \@ref(simplin-aovfit) -- it can be regarded as an *indicator of the strength of the linear relation between the variables*. ($R^2=1$ is a perfect linear fit -- all the points lay in a line -- and $R^2=0$ is the poorest fit.)
...
3. **Visualize the fitted regression line**.
    ...
    * Go to `'Graphs' -> 'Scatterplot...'`. A window with two panels will pop-up (Figures \@ref(fig:scatter1) and \@ref(fig:scatter2)).
        ...
        ```{r, scatter1, echo = FALSE, out.width = '45%', fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:scatter1title)', cache = TRUE}
        knitr::include_graphics("images/screenshots/scatterplot1.png")
        ```

        ```{r, scatter2, echo = FALSE, out.width = '45%', fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:scatter2title)', cache = TRUE}
    ...
    * The following `R` code will be generated. It produces a scatterplot of `MathMean` vs `logGDPp`, with its corresponding regression line.
        ```{r, echo = FALSE, warning = FALSE, cache = TRUE}
        library(splines)
        library(car)
        library(sandwich)
        ```
        ```{r, out.width = '70%', fig.asp = 1, fig.align = 'center', fig.pos = 'h!', cache = TRUE}
        ...
        ```{r, out.width = '70%', fig.asp = 1, fig.align = 'center', fig.pos = 'h!', results = 'hide', cache = TRUE}
        scatterplot(MathMean ~ logGDPp, reg.line = lm, smooth = FALSE, spread = FALSE,
...
### Case study II: Apportionment in the EU and US {#simplin-examps-euus}
...
(ref:UStitle) Correct importation of the `US` dataset.

(ref:EUtitle) The `EU` dataset with `Country` set as the case names.
...
1. The `US_apportionment.xlsx` file ([download](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/US_apportionment.xlsx)) contains the 50 US states entitled to representation. The variables are `State`, `Population2010` (from the last census) and `Seats2013–2023`. This is an Excel file that we can read using `'Data' -> 'Import data' -> 'from Excel file...'`. A window will pop-up, asking for the right options. We set them as in Figure \@ref(fig:excel), since we want the variable `State` to be the case names. After clicking in `'View dataset'`, the data should look like Figure \@ref(fig:US).
    ...
    ```{r, excel, echo = FALSE, out.width = '45%', fig.align = 'center', fig.pos = 'h!', fig.cap = 'Importation of an Excel file.', cache = TRUE}
    knitr::include_graphics("images/screenshots/excel.png")
    ```

    ```{r, US, echo = FALSE, out.width = '45%', fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:UStitle)', cache = TRUE}
...
(ref:newvartitle) Creation of the new variable `RatioSeats2013.2023`. The expression to compute is `Seats2013.2023/sum(Seats2013.2023)`.
...
3. Create the variable `RatioSeats2013.2023` as shown in Figure \@ref(fig:newvar). Be **careful to not overwrite** the variable `Seats2013.2023`.
    ...
    ```{r, newvar, echo = FALSE, out.width = '45%', fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:newvartitle)', cache = TRUE}
...
summary(appUS)
```
The fitted regression line is `RatioSeats2013.2023` $=0.000+1.005\,\times$`RatioPopulation2010` and has an $R^2=0.9991$ (`'Multiple R-squared'`), which means that the data is almost perfectly linearly distributed. Furthermore, the *intercept coefficient is not significant for the regression*. This is seen in the column `'Pr(>|t|)'`, which gives the $p$-values for the null hypotheses $H_0:\beta_0=0$ and $H_0:\beta_1=0$, respectively. The null hypothesis $H_0:\beta_0=0$ is *not rejected* ($p\text{-value}=0.407$; non-significant) whereas $H_0:\beta_1=0$ is rejected ($p\text{-value}=0$; significant)^[We will be able to say more about how these test are performed after Section \@ref(simplin-inference).]. Hence, we can conclude that the appropriation of seats in the US House of Representatives is indeed directly proportional to the population of each state (partially answers Q1).
...
```{r, ratios, echo = FALSE, out.width = '70%', fig.asp = 1, results = 'hide', fig.align = 'center', fig.pos = 'h!', fig.cap = 'The apportionment in the US House of Representatives compared with a linear fit.', cache = TRUE}
...
(ref:mscatter1atitle) Scatterplot matrix window, `'Data'` panel.

```{r, mscatter1a, echo = FALSE, out.width = '45%', fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:mscatter1atitle)', cache = TRUE}
knitr::include_graphics("images/screenshots/mscatter1a.png")
```

(ref:mscatter1btitle) Scatterplot matrix window, `'Options'` panel. Be sure to tick the `'Least-squares line'` box in order to display the fitted regression line.

```{r, mscatter1b, echo = FALSE, out.width = '45%', fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:mscatter1btitle)', cache = TRUE}
knitr::include_graphics("images/screenshots/mscatter1b.png")
```

(ref:mscatter2title) Scatterplot matrix for the variables `CamCom2011`, `Population2010` and `Seats2011` of `EU` dataset with boxplots in the central panels.

```{r, mscatter2, echo = FALSE, out.width = '70%', fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:mscatter2title)', cache = TRUE}
...
(ref:seats2011title) `Seats2011` vs `Population2010` in the `EU` dataset.

```{r, seats2011, echo = FALSE, results = 'hide', out.width = '70%', fig.asp = 1, fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:seats2011title)', cache = TRUE}
...
(ref:seats2011btitle) `Seats2011` vs `Population2010` in the `EU` dataset, Germany excluded.

```{r, seats2011btitle, echo = FALSE, results = 'hide', out.width = '70%', fig.asp = 1, fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:seats2011btitle)', cache = TRUE}
scatterplot(Seats2011 ~ Population2010, reg.line = lm, smooth = FALSE, spread = FALSE,
            id.method = 'mahal', id.n = 25, id.cex = 0.75, boxplots = FALSE, span = 0.5,
            ellipse = FALSE, levels = c(.5, .9),
...
(ref:seats2011ctitle) `CamCom2011` vs `Population2010` in the `EU` dataset, Germany excluded.

```{r, seats2011ctitle, echo = FALSE, results = 'hide', out.width = '70%', fig.asp = 1, fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:seats2011ctitle)', cache = TRUE}
...
### The `lm` function {#simplin-R-lm}

We are going to employ the `EU` dataset from Section \@ref(simplin-examps-euus), with the case names set as the `Country`. In case you do not have it loaded, you can download it [here](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/EU.RData) as an `.RData` file.
...
### Simple computations {#simplin-R-comput}
...
### Variables and assignment {#simplin-R-variables}
...
### Some functions {#simplin-R-functions}
...
### Matrices, data frames and lists {#simplin-R-dataframes}
...
## Model formulation and estimation by least squares {#simplin-model}
...
```{r, leastsquares, echo = FALSE, fig.cap = '(ref:leastsquarestitle)', screenshot.alt = "images/screenshots/least-squares.png", dev = 'png', cache = TRUE, fig.align = 'center', fig.pos = 'h!', out.width = '90%'}
...
## Assumptions of the model {#simplin-assumps}
...
```{r, linearmodel, echo = FALSE, out.width = '70%', fig.align = 'center', fig.pos = 'h!', fig.cap = 'The key concepts of the simple linear model. The yellow band denotes where the $95\\%$ of the data is, according to the model.', cache = TRUE}
...
Figures \@ref(fig:linearmodelgood) and \@ref(fig:linearmodelbad) represent situations where the assumptions of the model are respected and violated, respectively. For the moment, we will focus on building the intuition for checking the assumptions visually. In Chapter \@ref(multlin) we will see more sophisticated methods for checking the assumptions. We will see also what are the possible fixes to the failure of assumptions.

```{r, linearmodelgood, echo = FALSE, out.width = '70%', fig.align = 'center', fig.pos = 'h!', fig.cap = 'Perfectly valid simple linear models (all the assumptions are verified).', cache = TRUE}
knitr::include_graphics("images/R/linearmodelgood.png")
```

```{r, linearmodelbad, echo = FALSE, out.width = '70%', fig.align = 'center', fig.pos = 'h!', fig.cap = 'Problematic simple linear models (a single assumption does not hold).', cache = TRUE}
...
## Inference for the model coefficients {#simplin-inference}
...
### Distributions of the fitted coefficients {#simplin-inference-dists}
...
```{r, randomcoefs, echo = FALSE, fig.cap = '(ref:randomcoefstitle)', screenshot.alt = "images/screenshots/lm-random.png", dev = 'png', cache = TRUE, fig.align = 'center', fig.pos = 'h!', out.width = '90%'}
knitr::include_app('https://ec2-35-177-34-200.eu-west-2.compute.amazonaws.com/lm-random/', height = '1000px')
```

The problem with \@ref(eq:norm1) and \@ref(eq:se1) is that *$\sigma^2$ is unknown* in practice, so we need to estimate $\sigma^2$ from the data. We do so by computing the *sample variance of the residuals* $\hat\varepsilon_1,\ldots,\hat\varepsilon_n$. First note that *the residuals have zero mean*. This can be easily seen by replacing $\hat\beta_0=\bar Y-\hat\beta_1\bar X$:
\begin{align}
...
\end{align}
Due to this, we can and we can do it by computing a *rescaled* sample variance of the residuals:
\begin{align*}
...
### Confidence intervals for the coefficients {#simplin-inference-cis}
...
(ref:ttesttitle) The Student's $t$ distribution for the $t$-statistics associated to null intercept and slope, for the `y1 ~ x1` regression of the `assumptions` dataset.

```{r, ttest, echo = FALSE, out.width = '70%', fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:ttesttitle)', cache = TRUE}
...
(ref:cititle) Illustration of the randomness of the CI for $\beta_0$ at $100(1-\alpha)\%$ confidence. The plot shows 100 random CIs for $\beta_0$, computed from 100 random datasets generated by the same linear model, with intercept $\beta_0$. The illustration for $\beta_1$ is completely analogous. Application also available [here](https://ec2-35-177-34-200.eu-west-2.compute.amazonaws.com/ci-random/).

```{r, ci, echo = FALSE, fig.cap = '(ref:cititle)', screenshot.alt = "images/screenshots/ci-random.png", dev = 'png', cache = TRUE, fig.align = 'center', fig.pos = 'h!', out.width = '90%'}
...
### Testing on the coefficients {#simplin-inference-tests}
...
The tests for significance are built-in in the `summary` function, as we glimpsed in Section \@ref(simplin-examps-euus). For `mod1`, we have:
```{r, collapse = TRUE, cache = TRUE}
summary(mod1)
```
...
- **Variance**. The variance that determines the length of the CI is $\frac{\hat\sigma^2}{n}\left(1+\frac{(x-\bar x)^2}{s_x^2}\right)$. Its interpretation is very similar to the one given for $\hat\beta_0$ and $\hat\beta_1$ in Section \@ref(simplin-inference):
...
```{r, cipred, echo = FALSE, fig.cap = '(ref:cipredtitle)', screenshot.alt = "images/screenshots/ci-prediction.png", dev = 'png', cache = TRUE, fig.align = 'center', fig.pos = 'h!', out.width = '90%'}
...
## ANOVA and model fit {#simplin-aovfit}

### ANOVA {#simplin-aovfit-anova}

As we have seen in Sections \@ref(simplin-inference) and \@ref(simplin-prediction), the variance of the error, $\sigma^2$, plays a fundamental role in the inference for the model coefficients and prediction. In this section we will see how the variance of $Y$ is decomposed into two parts, each one corresponding to the regression and to the error, respectively. This decomposition is called the *ANalysis Of VAriance* (ANOVA).
...
- $\text{SSR}=\sum_{i=1}^n\left(\hat Y_i-\bar Y\right)^2$, the **regression sum of squares**^[Recall that SSR is different from RSS (Residual Sum of Squares, Section \@ref(simplin-model)).]. This is the variation explained by the regression line, that is, *the variation from $\bar Y$ that is explained by the estimated conditional mean $\hat Y_i=\hat\beta_0+\hat\beta_1X_i$*. $\text{SSR}=ns_{\hat y}^2$, where $s_{\hat y}^2$ is the sample variance of $\hat Y_1,\ldots,\hat Y_n$.
...
```{r, anova, echo = FALSE, out.width = '70%', fig.align = 'center', fig.pos = 'h!', fig.cap = 'Visualization of the ANOVA decomposition. SST measures the variation of $Y_1,\\ldots,Y_n$ with respect to $\\bar Y$. SST measures the variation with respect to the conditional means, $\\hat \\beta_0+\\hat\\beta_1X_i$. SSE collects the variation of the residuals.', fig.show = 'hold'}
...
```{r, anovaillus, echo = FALSE, fig.cap = '(ref:anovaillustitle)', screenshot.alt = "images/screenshots/anova.png", dev = 'png', cache = TRUE, fig.align = 'center', fig.pos = 'h!', out.width = '90%'}
...
\end{align*}
that is, the hypothesis of no linear dependence of $Y$ on $X$. The result of this test is completely equivalent to the $t$-test for $\beta_1$ that we saw in Section \@ref(simplin-inference) (this is something *specific for simple linear regression* -- the $F$-test will not be equivalent to the $t$-test for $\beta_1$ in Chapter \@ref(multlin)). It happens that
\begin{align*}
...
Trusting blindly the $R^2$ can lead to catastrophic conclusions, since the model may not be correct. Here is a counterexample of a linear regression performed in a data that clearly does not satisfy the assumptions discussed in Section \@ref(simplin-assumps), but despite so it has a large $R^2$. Recall how biased will be the predictions for $x=0.35$ and $x=0.65$!
...
## Nonlinear relationships {#simplin-nonlin}
...
```{r, nonlineartransf, echo = FALSE, warning = FALSE, results = 'hide', out.width = '45%', fig.show = 'hold', fig.asp = 1, fig.align = 'center', fig.pos = 'h!', fig.cap = 'Some common nonlinear transformations and their negative counterparts. Recall the domain of definition of each transformation.', cache = TRUE}
x <- seq(-2, 5, l = 200)
...
```{r, transf, echo = FALSE, fig.cap = '(ref:transftitle)', screenshot.alt = "images/screenshots/non-linear.png", dev = 'png', cache = TRUE, fig.align = 'center', fig.pos = 'h!', out.width = '90%'}
knitr::include_app('https://ec2-35-177-34-200.eu-west-2.compute.amazonaws.com/non-linear/', height = '550px')
```

```{block2, type = 'rmdinsight'}
As you might have realized, applying nonlinear transformations to the predictors is a **simple trick that extends enormously the functionality of the linear model**. This is particularly useful in real applications, where linearity is hardly verified (for example, in the PISA case study of Section \@ref(simplin-examps-pisa), we employed
...
1. Through `R Commander`. In Section \@ref(simplin-examps-euus) we saw how to create a new variable in our active dataset (remember Figure \@ref(fig:newvar)). Go to `'Data' -> 'Manage variables in active dataset...' -> 'Compute new variable...'`. Set the `'New variable name'` to `x2` and the `'Expression to compute'` to `x^2`.
...
## Exercises and case studies {#simplin-exercises}

### Data importation {#simplin-exercises-dataimp}
...
### Simple data management {#simplin-datamanag}
...
### Model formulation and estimation {#simplin-exercises-model}
...
### Assumptions of the linear model {#simplin-exercises-assumps}
...
<!--
### Inference {#simplin-exercises-inference}

```{block, type = 'rmdexercise'}
TBA
```

### Prediction {#simplin-exercises-prediction}

```{block, type = 'rmdexercise'}
TBA
```

### ANOVA and model fit {#simplin-exercises-aovfit}
...
### Nonlinear relations {#simplin-exercises-nonlin}
...
### Case study: *Moore's law* {#simplin-exercises-moore}
...
### Case study: *Growth in a time of debt* {#simplin-exercises-growth}
...
(ref:reinharttitle) The *magical threshold* of $90\%$ external debt.

```{r, reinhart, echo = FALSE, out.width = '45%', fig.align = 'center', fig.pos = 'h!', fig.cap = '(ref:reinharttitle)', cache = TRUE}
...
```{r, video, echo = FALSE, fig.align = 'center', fig.pos = 'h!', fig.cap = 'CNN interview to Paul Krugman (key point from 1:16 to 1:40), broadcasted in 2013.', screenshot.opts = list(delay = 15), dev = 'png', cache = TRUE}
