...
- **Variance**. The variances $\mathrm{SE}(\hat\beta_0)^2$ and $\mathrm{SE}(\hat\beta_1)^2$ have an interesting interpretation in terms of its components:
    ...
    - *Error variance $\sigma^2$*. The more disperse the error is, the less precise the estimates are.
...
$$
and it can be seen that $R^2=r_{xy}^2$. Interestingly, it also holds that $R^2=r^2_{y\hat y}=\frac{s_{y\hat y}}{s_ys_{\hat y}}$, that is, *the square of the sample correlation coefficient between $Y_1,\ldots,Y_n$ and $\hat Y_1,\ldots,\hat Y_n$ is $R^2$*, a fact that is not immediately evident. This fact can be visualized with the next code.
