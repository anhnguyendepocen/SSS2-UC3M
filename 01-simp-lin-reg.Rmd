...
```{block, type = 'rmdnote', out.width = '90%'}
...
2. Go to `'Data' -> 'Manage variables in active dataset...' -> 'Compute new variables'`. 
...
summary(appUS)
```
The fitted regression line is `RatioSeats2013.2023`$=0.000+1.005\,\times$`RatioPopulation2010` and has an $R^2=0.9991$ (`'Multiple R-squared'`), which means that the data is almost perfectly linearly distributed. Furthermore, the *intercept coefficient is not significant*. This is seen in the column `'Pr(>|t|)'`, which gives the $p$-values for the null hypotheses $H_0:\beta_0=0$ and $H_0:\beta_1=0$, respectively. The null hypothesis $H_0:\beta_0=0$ is *not rejected* ($p$-value$=0.407$; non-significant) whereas $H_0:\beta_1=0$ is rejected ($p$-value$=0$; significant)^[We will be able to say more about how these test are performed after Section \@ref(sec:inference).]. Hence, we can conclude that the appropriation of seats in the US House of Representatives is indeed directly proportional to the population of each state (partially answers Q1).
...
By this time you probably had realized that some pieces of `R` code are repeated over and over, and that it is simpler to just modify them than to navigate the menus. For example, the codes `lm` and `scatterplot` always appear related with linear models and scatterplots. It is important to know some of the `R` basics in order to understand what are these pieces of text actually doing. Do not worry, the menus will always be there...

In the following sections, **type** -- not copy and paste systematically -- the code in the `'R Script'` panel and send it to the output panel (on the selected expression, either with the `'Submit'` button or with `'Control' + 'R'`). You should get the same outputs (which are preceded by `## [1]`).

### Simple computations

```{r, echo = TRUE, error = TRUE, collapse = TRUE}
# This is a comment - it is always initiated by a hashtag
# Its purpose is to annotate the code and it does nothing else
...
# Now the value of a is 2 and not 1, as before. There is no history of values for a variable.
a 
...
- Compute $x_2+y_4$ and $\cos(x_3) + \sin(x_2) e^{y_2}$.
- Set $x_3=0$ and $y_58=-1$. Recompute the previous expressions.
- Index $y$ by $x$. What is the output?
```
...
```{r, echo = TRUE, collapse = TRUE, error = TRUE}
# Data
myData <- c(1.04, 0.53, 4.91, 5.53, -1.50, 5.17, 6.35)
...
```{r, collapse = TRUE, error = TRUE}
# lm has the syntax: lm(formula = y ~ x, data = data)
...
# Summary of the model (also a list)
sumMod <- summary(mod)
sumMod

# We use max.level = 1 to avoid lengthy outputs
names(sumMod)
...
- What are the 5 largest residuals? What are the 5 lowest? What is the interpretation in terms of the `UE` dataset?
```
...
$$
In other words, we look fot the estimators $(\hat\beta_0,\hat\beta_1)$ such that
$$
...
```{block, type = 'rmdimportant'}
The *population regression coefficients*, $(\beta_0,\beta_1)$, **are not the same** as the *estimated regression coefficients*, $(\hat\beta_0,\hat\beta_1)$. $(\beta_0,\beta_1)$ are the theoretical and **always** unknown quantities (except under controlled scenarios). $(\hat\beta_0,\hat\beta_1)$ are computed from the data, in particular, they are the output of `lm`. In an abuse of notation, the term *regression line* is often used to denote both the *theoretical* ($y=\beta_0+\beta_1x$) and the *estimated* ($y=\hat\beta_0+\hat\beta_1x$) regression lines.
```
...
```{block, type = 'rmdnote'}
Do the same checkings for $X=$`ReadingMean` and $Y=$ `MathMean` from the `pisa` dataset.
```
...
Nothing is said about the randomness of $X$. The linear model assumes that $Y$ is continuous, but $X$ can be discrete, factorial (as we will see in the next chapter).
...
2. **Homocedasticity**: $\varepsilon_i\sim\mathcal{N}(0,\sigma^2)$ $\sigma^2$ is constant, it does not depend on $X$.
3. **Normality**: $\varepsilon\sim
4. **Independence**:

A good one-line summary of the model is
$$
Y|X=x\sim \mathcal{N}(\beta_0+\beta_1x,\sigma^2) \text{ (independence is implicit)}
$$

Note that there is nothing assumed about the distribution of $X$! So $X$ could be discrete or continuous, with any kind of distribution. But $Y|X$ has to be normal!


<!--
PISA back! Say something about variability

Do matrix scatterplot. Go to  
...
Normality, null hypothesis, test on the coefficients
...
Germany exlcluded, prediction with it
Ratios and prediction of Seats2014 according to CamCom2011

Confidence intervals - different if `x` is random or non-random.
 was observed or not, or if is random or nor.
Prediction of a new observation can be done via the function `predict`, which also provides confidence intervals. The `newdata` argument of `predict` needs a `data.frame`.
...
## Full analysis: 'Growth in a time of debt'
