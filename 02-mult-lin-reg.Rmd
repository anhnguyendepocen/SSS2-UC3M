...
2. **Graphically**. Make a scatterplot matrix with all the variables. Add the `'Least-squares lines'`, `'Histograms'` on the diagonals and choose to identify 2 points.
    ...
    ```{r, scat, collapse = TRUE, out.width = '90%', fig.asp = 1, outfig.align = 'center', fig.cap = 'Scatterplot matrix for `wine`.'}
...
> Can we *combine* these simple regressions to increase the $R^2$ and the prediction accuracy for `Price`?
...
\end{align*}
If the model is named `modFull`, we get the following summary when clicking in `'OK'`:
```{r, collapse = TRUE}
modFull <- lm(Price ~ Age + AGST + FrancePop + HarvestRain + WinterRain, data = wine)
summary(modFull)
```
...
```{r, collapse = TRUE}
modWine <- lm(Price ~ . - FrancePop, data = wine)
summary(modWine)
```
All the coefficients are significant at level $\alpha=0.05$. Therefore, there is no clear redundant information. In addition, the $R^2$ is very similar to the full model, but the `'Adjusted R-squared'`, a weighting of the $R^2$ to account for the number of predictors used by the model, is slightly larger. Hence, this means that, comparatively to the number of predictors used, `modWine` explains more variability of `Price` than `modFull`. Later in this chapter we will see the precise meaning of the $R^2$ adjusted.

The comparison of the coefficients of both models can be done with `'Models -> Compare model coefficients...'`:
```{r, collapse = TRUE}
compareCoefs(modFull, modWine)
```
Note how **the coefficients for `modWine` have smaller errors than `modFull`**.

As a conclusion, `modWine` is a model that explains the $82.75\%$ of the variability in a non-redundant way and with all their coefficients significant. Therefore, we have a formula for effectively explaining and predicting the quality of a vintage (answers Q1).

The interpretation of `modWine` agrees with well-known facts in viticulture that make perfect sense (Q2):
...
```{block, type = 'rmdexercise'}
Create a new variable in `wine` named `PriceOrley`, defined as `Price - 8.4937`. Check that the model `PriceOrley ~ . - FrancePop - Price` *kind of* coincides with the formula given in the second paragraph of the [Financial Times article](http://www.ft.com/cms/s/0/1e9cb152-5824-11dc-8c65-0000779fd2ac.html?ft_site=falcon&desktop=true#axzz4NGeRjzkc) (the author forgot to add the `Age` term and the `ACGS` has an extra zero).
```
...
```{r, scat2, collapse = TRUE, out.width = '90%', fig.asp = 1, outfig.align = 'center', fig.cap = 'Scatterplot matrix for `crim`, `dis`, `medv`, `nox` and `rm` from the `Boston` dataset.'}
