...
```{r, anova3D, echo = FALSE, out.width = '70%', fig.align = 'center', fig.cap = 'Visualization of the ANOVA decomposition when $k=2$. SST measures the variation of $Y_1,\\ldots,Y_n$ with respect to $\\bar Y$. SST measures the variation with respect to the conditional means, $\\hat\\beta_0+\\hat\\beta_1X_{i1}+\\hat\\beta_2X_{i2}$. SSE collects the variation of the residuals.', fig.show = 'hold'}
knitr::include_graphics("images/R/anova3D.png")
```

```{r, anovaillus3D, echo = FALSE, fig.cap = 'Illustration of the ANOVA decomposition and its dependence on $\\sigma^2$ and $\\hat\\sigma^2$.', screenshot.opts = list(delay = 10), dev = 'png', cache = TRUE, fig.align = 'center', out.width = '90%'}
knitr::include_app('https://egarpor.shinyapps.io/anova-3D/', height = '900px')
```
...
```{r, R2, echo = FALSE, out.width = '70%', fig.align = 'center', fig.cap = 'Comparison of $R^2$ and $R^2_{\\text{Adj}}$ for $n=200$ and $k$ ranging from $1$ to $198$. $M=100$ datasets were simulated with **only the first two** predictors being significant. The thicker curves are the mean of each color\'s curves.', fig.show = 'hold'}
...
- If $k=1$, we need at least $n=2$ points to fit uniquely a line. However,this line is depends entirely on two random points, $\hat\sigma^2=\infty$. Therefore we need at least $n=3$ points, or in other words $n\geq k + 2=3$.
- If $k=2$, we need at least $n=3$ points to fit uniquely a plane. But this plane depends entirely on two random points and as a consequence $\hat\sigma^2=\infty$. Therefore we need $n\geq k + 2=4$.
...
```{r, dft, echo = FALSE, warning = FALSE, results = 'hide', out.width = '70%', fig.show = 'hold', fig.asp = 1, fig.align = 'center', fig.cap = 'Effect of $\\text{df}=n-k-1$ in $t_{\\text{df};\\alpha/2}$ for $\\alpha=0.10,0.05,0.01$.'}
df <- 1:30
alpha <- 0.10
...
```{r, bic, echo = FALSE, out.width = '70%', fig.align = 'center', fig.cap = 'Comparison of BIC and AIC for $n=200$ and $k$ ranging from $1$ to $198$. $M=100$ datasets were simulated with **only the first two** predictors being significant. The thicker curves are the mean of each color\'s curves.', fig.show = 'hold'}
...
```{r, diagnostics1, echo = FALSE, out.width = '90%', fig.align = 'center', fig.cap = 'Residuals vs. fitted values plots for datasets respecting (left column) and violating (right column) the linearity assumption.'}
knitr::include_graphics("images/R/diagnostics1.png")
```

```{r, diagnostics2, echo = FALSE, out.width = '90%', fig.align = 'center', fig.cap = 'QQ-plots for datasets respecting (left column) and violating (right column) the normality assumption.'}
knitr::include_graphics("images/R/diagnostics2.png")
```

```{r, diagnostics3, echo = FALSE, out.width = '90%', fig.align = 'center', fig.cap = 'Scale-location plots for datasets respecting (left column) and violating (right column) the homoscedasticity assumption..'}
...
# Correlations - none is seems suspicius
cor(data)
```
...
\]
where $R^2_{X_j|X_{-j}}$ is the $R^2$ from a regression of $X_j$ into the remaining predictors. This gives a direct insight into which predictors are multicollinear with the next rules-of-thumb:
