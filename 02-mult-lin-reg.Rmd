...
Figure \@ref(fig:leastsquares2) illustrates the geometrical interpretation of a multiple linear model: a plane in the $k$-dimensional space. If $k=1$, the plane is the regression line for simple linear regression. If $k=2$, then the plane can be visualized in a three-dimensional plot

```{r, leastsquares2, echo = FALSE, fig.cap = 'The least squares regression plane $y=\\hat\\beta_0+\\hat\\beta_1x_1+\\hat\\beta_2x_2$ and its dependence on the kind of distance in the error criterion.', screenshot.opts = list(delay = 10), dev = 'png', cache = TRUE, fig.align = 'center', out.width = '90%'}
...
- We denote by $\mathbf{X}=(X_1,\ldots,X_k)$ to the collection of random variables acting as predictors. $\mathbf{X}$ is a *random vector*. Hence, a sample of $(\mathbf{X},Y)$ is $(\mathbf{X}_1,Y_1),\ldots,(\mathbf{X}_n,Y_n)$. $\mathbf{X}_i=(X_{i1},\ldots,X_{ik})$ is the $i$-th observation of $\mathbf{X}$. $X_{ij}$ denotes the $i$-th observation of the $j$-th predictor $X_j$.
...
- The *vector of responses* $\mathbf{Y}$, the *vector of coefficients* $\boldsymbol\beta$ and *vector of errors* are, respectively^[The vectors are regarded as column matrices.],
\[
...
```{block2, type = 'rmdinsight'}
There are some similarities between \@ref(eq:k3) and $\hat\beta_1=(s_x^2)^{-1}s_{xy}$ from the simple linear model: both measure the covariance between $\mathbf{X}$ and $Y$ weighted by the variance of $\mathbf{X}$.
```
...
# Data
leastSquares3D <- data.frame(x1 = x1, x2 = x2, yLin = yLin, yQua = yQua, yExp = yExp)
```
...
Consider the multiple linear model $Y=\beta_0+\beta_1X_1+\beta_2X_2+\varepsilon$ and its associated simple linear models $Y=\alpha_0+\alpha_1X_1+\varepsilon$ and $Y=\gamma_0+\gamma_2X_2+\varepsilon$. Assume that we have a sample $(X_{11},X_{12},Y_1),\ldots, (X_{n1},X_{n2},Y_n)$. Then, in general, $\hat\alpha_0\neq\hat\beta_0$, $\hat\alpha_1\neq\hat\beta_1$, $\hat\gamma_0\neq\hat\beta_0$ and $\hat\gamma_1\neq\hat\beta_1$. This is: **the inclusion of a new predictor in general changes the coefficient estimates**.
```

```{r, multmarg, echo = FALSE, out.width = '90%', fig.align = 'center', fig.cap = 'The regression plane (blue) and its relation with the simple linear regressions (green lines). '}
...
Let's see how we can compute the CIs. We retake the `wine` dataset, so in case you do not have it loaded, you can download it [here](https://raw.githubusercontent.com/egarpor/SSS2-UC3M/master/datasets/wine.RData) as an `.RData` file. We analyse the CI for the coefficients of `Price ~ Age + WinterRain`.
```{r, collapse = TRUE}
# Fit model
...
```
In this example, the 95% confidence interval for $\beta_0$ is $(4.3067, 7.6594)$. For $\beta_1$ is $(0.0024, 0.0745)$ and $\beta_2$ is $(-0.0017, 0.0032)$. Therefore, we can say that, with a 95% confidence, *the coefficients of `Age` and `WinterRain` are non-significant*. But in Section \@ref(mult) we saw that they *were significant* in the model `Price ~ Age + AGST + HarvestRain + WinterRain`! How is this possible? The answer is that the presence of extra predictors affects the coefficient estimates, as we saw in Figure \@ref(fig:multmarg). Therefore, the precise statement to make is: **in the model `Price ~ Age + WinterRain`**, with a 95% confidence, the coefficients of `Age` and `WinterRain` are non-significant. Note that this **does not** mean that they will be always non-significant: in `Price ~ Age + AGST + HarvestRain + WinterRain` they become significant.
...
To illustrate the use of `predict`, we retake the `wine` dataset. 
...
The RSS always decreases as more variables are added to the mode, hence the $R^2$ always increases as more variables are added. Why is so? Because the flexibility of the model increases.
...
\]
Once all of the correct variables have been included in the mode, adding additional *noise* variables will lead to only a very small decrease in RSS
