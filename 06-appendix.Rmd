...
The useful commands for performing **logistic regression** are given in the next table. We assume that:
...
| **CIs predicted probability** | Not inmediate. Use `predictCIsLogistic(model, newdata = new, level = level)` as seen in Section \@ref(log-reg-prediction) |
| **Variable selection** | `stepwise(model)` |
| **Multicollinearity detection** | `vif(model)` |
| **$R^2$** | Not inmediate. Use `r2Log(model = model)` as seen in Section \@ref(log-reg-selection) |
